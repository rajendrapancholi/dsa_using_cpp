...
Following the completion of the first two jobs, the next shortest job with a duration of 3 will be executed with a waiting time of 3 (1 + 2).
Then, the job with a duration of 4 will be executed with a waiting time of 6 (1 + 2 + 3).
Finally, the job with the longest duration of 7 will be executed with a waiting time of 10 (1 + 2 + 3 + 4).Hence, the average waiting time is calculated as (0 + 1 + 3 + 6 + 10) / 5 = 20 / 5 = 4.
*/
...
class Solution {
public:
  int sjfAvgWT(vector<int> &j){
    sort(j.begin(), j.end());
    int n = j.size(), et = 0, wt = 0, tt = 0;
    for(int i = 0; i < n; i++){
      wt += tt;
      tt += j[i];
    }
...
int main() {
  vector<int> jobs = {3, 1, 4, 2, 5}; // Output: 4           
  Solution sl;
  cout<<sl.sjfAvgWT(jobs)<<endl;
  return 0;
}
